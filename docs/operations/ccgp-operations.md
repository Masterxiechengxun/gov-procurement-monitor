...
系统内置两个数据来源：**中国政府采购网（ccgp）** 和 **四川省公共资源交易网（sichuan_ggzy）**，用户可在小程序「设置」→「采购网站」中独立开关。
...
> `kw` 和 `region` 参数仅对 CCGP 生效（用于搜索接口过滤），四川省公共资源交易网按自身逻辑全量抓取。
...
[CCGP] 第1页获取 N 条
[Crawl] 去重后新增 M 条
[Crawl] 开始抓取: 四川省公共资源交易网
[Crawl] 四川省公共资源交易网 使用 start_time=YYYY-MM-DD
[Crawl] 去重后新增 K 条
[Crawl] 执行完成
```

**验证通过条件：**
- 日志中两个来源均出现（`中国政府采购网` 和 `四川省公共资源交易网`）
- CCGP 日志中有 `总页数=X` 说明分页解析正常
- 至少有一个来源 `新增 M 条` > 0
- 没有 `CCGP反爬拦截` 警告（若有，等待 10 分钟后重试）

**若只出现一个来源：**
说明用户在小程序「设置」→「采购网站」中只启用了一个来源，进入设置页打开两个来源的开关后重试。
...
以下参数均通过 crawl 的 event 传入，**仅对中国政府采购网（CCGP）生效**：
...
四川省公共资源交易网按自身接口逻辑全量抓取，不受 `kw`/`region` 参数影响，`start_time` 由 `detectStartDate` 自动检测。

定时任务自动触发时，`kw` 和 `region` 参数使用云函数代码中的默认值（`kw="耗材"`，`region="四川"`）。
...
**症状：** 抓取日志中只出现一个来源，另一个来源完全没有日志

**原因：** 用户在小程序「设置」→「采购网站」中将该来源的开关关闭，`custom_sources` 只保存了启用的来源。

**处理方式：**
1. 进入小程序「设置」→「采购网站」
2. 确认两个来源的开关均为开启状态（显示绿色「抓取中」标签）
3. 若某来源为关闭状态，打开开关，等待保存成功后重新触发抓取
