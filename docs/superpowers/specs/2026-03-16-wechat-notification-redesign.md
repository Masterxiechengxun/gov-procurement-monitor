...
<div style="margin-bottom:15px;">
  <p><b>{序号}. {highlightKeywords(item.title, item.matchedKeywords)}</b></p>
...
标题中命中的关键字使用红色标注，由 `highlightKeywords(title, matchedKeywords)` 实现：将 `matchedKeywords` 中每个关键字在 title 中的出现位置替换为 `<span style="color:#FF3B30;">{关键字}</span>`。若标题中某关键字出现多次，每次都标红；若 `matchedKeywords` 为空，则直接输出 `escapeHtml(title)`，不做任何处理。
...
- 若 `stats.newItems > 0`：显示"共抓取 N 条，新增 M 条（无关键字命中）"
  — 含义：系统有新入库数据，但当前用户的关键字没有命中（`userItems.length === 0` 已由场景 3 触发条件保证）
...
- `highlightKeywords(title, matchedKeywords)` — 将标题中命中的关键字替换为红色 `<span>`，matchedKeywords 为空时返回 `escapeHtml(title)`
...
module.exports = {
    ...
    highlightKeywords: highlightKeywords,   // 导出供单元测试
...
对每个用户（uid, config）：
  ...
  2. // results._matchedNewItems 由 deduplicateAndSave 在抓取阶段动态附加，
