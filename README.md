...
政府采购信息智能抓取与推送的微信小程序，基于腾讯云开发（云函数 + 云数据库）。

---

## 功能特性

- **全量抓取**：每天定时自动抓取政府采购网站的所有采购公告，确保数据完整性
- **双来源**：内置中国政府采购网（CCGP）和四川省公共资源交易网，可在设置页独立开关
- **智能匹配**：抓取入库时用所有用户的合并关键字打标签，方便前端快速筛选展示
- **扁平关键字**：用户管理设备关键字，每个关键字即独立分类，自动同步为首页筛选标签
- **精准推送**：按每个用户自己的关键字独立过滤，只推送该用户真正关注的采购信息（PushPlus）
- **详情 web-view**：详情页通过 web-view 直接加载原始公告网页，无需本地存储 HTML
...
├── cloudfunctions/
│   ├── crawl/                      # 抓取云函数（定时 + 手动）
│   │   ├── crawlers/ccgp.js        # 中国政府采购网爬虫
│   │   ├── crawlers/sichuan-ggzy.js# 四川省公共资源交易网爬虫
